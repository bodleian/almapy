...
def _make_response(status_code: int, body: str = "", content_type: str = "application/json") -> httpx.Response:
...
class TestExecuteOutcomeRecording:
    @pytest.mark.asyncio
    async def test_success_calls_record_success(self, client: AlmaClient, mock_http: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_retryable_error_calls_record_failure(self, client: AlmaClient, mock_http: AsyncMock) -> None:
    ...
    async def test_non_retryable_4xx_does_not_call_record_failure(
        ...
        import json
        body = json.dumps({"errorList": {"error": [{"errorCode": "401861", "errorMessage": "User not found."}]}})
    ...
    async def test_5xx_calls_record_failure(self, client: AlmaClient, mock_http: AsyncMock) -> None:
        import json
        body = json.dumps({"errorList": {"error": [{"errorCode": "500", "errorMessage": "Server error"}]}})
